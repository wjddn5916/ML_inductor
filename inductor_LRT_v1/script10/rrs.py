...
def run_simul(version_idx_str):
    ...
    os.makedirs(f'.\script10\ML\SIMUL_{version_idx_str}')


    #2 Make Variable info file
    with open(f'.\script10\ML\SIMUL_{version_idx_str}\info.yaml', "w") as info_file:
    ...
    #Save file
    with open(f'.\\script10\\ML\\SIMUL_{version_idx_str}\\run_ansys_{version_idx_str}.py',"w") as f :
        f.write(ref_script_str)


    #4 make batch file.
    filepath2 = os.path.join('ML',folder_name,f'run_bat_{version_idx_str}.bat')
    with open(f'.\\script10\\ML\\SIMUL_{version_idx_str}\\run_bat_{version_idx_str}.bat',"w") as f :
        f.write(f'"C:\\Program Files\\AnsysEM\\AnsysEM21.1\\Win64\\ansysedt.exe" -iconic -runscriptandexit ".\\run_ansys_{version_idx_str}.py"')


    workingDir = f'.\\script10\\ML\\SIMUL_{version_idx_str}'
    executeFile = f'Y:\\git\\ML_inductor\\inductor_LRT_v1\\script10\\ML\\SIMUL_{version_idx_str}\\run_bat_{version_idx_str}.bat'
    os.chdir(workingDir)
    try :
    ...
    data1 = np.loadtxt(f'Z:\Autosimul_data\inductor\inductor_LRT_v1\{COMPUTER_NAME}\script10\inductance.csv', delimiter=",")
    new_data1 = np.vstack((data1, temp1))
    np.savetxt(f'Z:\Autosimul_data\inductor\inductor_LRT_v1\{COMPUTER_NAME}\script10\inductance.csv',new_data1,delimiter=",")

    data2 = np.loadtxt(f'Z:\Autosimul_data\inductor\inductor_LRT_v1\{COMPUTER_NAME}\script10\loss.csv', delimiter=",")
    new_data2 = np.vstack((data2, temp2))
    np.savetxt(f'Z:\Autosimul_data\inductor\inductor_LRT_v1\{COMPUTER_NAME}\script10\loss.csv',new_data2,delimiter=",")
