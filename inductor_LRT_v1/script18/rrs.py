...
def run_simul(version_idx_str):
    ...
    executeFile = f'Y:\\git\\ML_inductor\\inductor_LRT_v1\\script188\\ML\\SIMUL_{version_idx_str}\\run_bat_{version_idx_str}.bat'
    os.chdir(workingDir)
    try :
    ...
    temp1 = pd.read_csv(f'Y:\git\\ML_inductor\inductor_LRT_v1\script188\ML_data\inductance_{version_idx_str}.csv', sep=",")
    temp1 = temp1.to_numpy()
    
  
    temp2 = pd.read_csv(f'Y:\git\ML_inductor\inductor_LRT_v1\script188\ML_data\loss_{version_idx_str}.csv', sep=",")
    ...
    data1 = np.loadtxt(f'Z:\Autosimul_data\inductor\inductor_LRT_v1\{COMPUTER_NAME}\script188\inductance.csv', delimiter=",")
    new_data1 = np.vstack((data1, temp1))
    np.savetxt(f'Z:\Autosimul_data\inductor\inductor_LRT_v1\{COMPUTER_NAME}\script188\inductance.csv',new_data1,delimiter=",")

    data2 = np.loadtxt(f'Z:\Autosimul_data\inductor\inductor_LRT_v1\{COMPUTER_NAME}\script188\loss.csv', delimiter=",")
    new_data2 = np.vstack((data2, temp2))
    np.savetxt(f'Z:\Autosimul_data\inductor\inductor_LRT_v1\{COMPUTER_NAME}\script188\loss.csv',new_data2,delimiter=",")
...
for i in range(1, 10000): 
    ...
    try :
        try:
            os.remove(f'.\ML_aedt\ML188.aedt.lock')
        except:
            time.sleep(1)
        if os.path.isfile(f'.\ML_aedt\ML188.aedt') :
            os.remove(f'.\ML_aedt\ML188.aedt')
        time.sleep(1)	

        shutil.copy(f'.\ML_aedt\ML_ref.aedt',f'.\ML_aedt\ML188.aedt')
        time.sleep(1)
        ...
        shutil.rmtree(f'.\ML_aedt\ML188.aedtresults')
        try:
            os.remove(f'.\ML_aedt\ML188.aedt.lock')
        except:
            time.sleep(1)
