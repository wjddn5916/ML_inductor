...
def run_simul(version_idx_str):
    ...
    os.makedirs(f'.\script5\ML\SIMUL_{version_idx_str}')


    #2 Make Variable info file
    with open(f'.\script5\ML\SIMUL_{version_idx_str}\info.yaml', "w") as info_file:
    ...
    #Save file
    with open(f'.\\script5\\ML\\SIMUL_{version_idx_str}\\run_ansys_{version_idx_str}.py',"w") as f :
        f.write(ref_script_str)


    #4 make batch file.
    filepath2 = os.path.join('ML',folder_name,f'run_bat_{version_idx_str}.bat')
    with open(f'.\\script5\\ML\\SIMUL_{version_idx_str}\\run_bat_{version_idx_str}.bat',"w") as f :
        f.write(f'"C:\\Program Files\\AnsysEM\\AnsysEM21.1\\Win64\\ansysedt.exe" -iconic -runscriptandexit ".\\run_ansys_{version_idx_str}.py"')


    workingDir = f'.\\script5\\ML\\SIMUL_{version_idx_str}'
    executeFile = f'Y:\\git\\ML_inductor\\inductor_LRT_v1\\script5\\ML\\SIMUL_{version_idx_str}\\run_bat_{version_idx_str}.bat'
    os.chdir(workingDir)
    try :
    ...
    data1 = np.loadtxt(f'Z:\Autosimul_data\inductor\inductor_LRT_v1\{COMPUTER_NAME}\script5\inductance.csv', delimiter=",")
    new_data1 = np.vstack((data1, temp1))
    np.savetxt(f'Z:\Autosimul_data\inductor\inductor_LRT_v1\{COMPUTER_NAME}\script5\inductance.csv',new_data1,delimiter=",")

    data2 = np.loadtxt(f'Z:\Autosimul_data\inductor\inductor_LRT_v1\{COMPUTER_NAME}\script5\loss.csv', delimiter=",")
    new_data2 = np.vstack((data2, temp2))
    np.savetxt(f'Z:\Autosimul_data\inductor\inductor_LRT_v1\{COMPUTER_NAME}\script5\loss.csv',new_data2,delimiter=",")
...
for i in range(1, 10000): 
    ...
    try :
        try:
            os.remove(f'.\ML_aedt\ML5.aedt.lock')
        except:
            time.sleep(1)
        if os.path.isfile(f'.\ML_aedt\ML5.aedt') :
            os.remove(f'.\ML_aedt\ML5.aedt')
        time.sleep(1)	

        shutil.copy(f'.\ML_aedt\ML_ref.aedt',f'.\ML_aedt\ML5.aedt')
        time.sleep(1)
        ...
        shutil.rmtree(f'.\ML_aedt\ML5.aedtresults')
        try:
            os.remove(f'.\ML_aedt\ML5.aedt.lock')
        except:
            time.sleep(1)
