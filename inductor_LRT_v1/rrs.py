...
def run_simul(version_idx_str):
    ...
    height = 3 * N1 * (coil_width + move_z) / 2 + offset
    ...
    executeFile = f'Z:\\ANSYS_simulation\\inductor_practice\\ML\\SIMUL_{version_idx_str}\\run_bat_{version_idx_str}.bat'
    os.chdir(workingDir)
    try :
    ...
    temp1 = pd.read_csv(f'Z:\ANSYS_simulation\inductor_practice\inductance\inductance_{version_idx_str}.csv', sep=",")
    temp1 = temp1.to_numpy()
    
  
    temp2 = pd.read_csv(f'Z:\ANSYS_simulation\inductor_practice\loss\loss_{version_idx_str}.csv', sep=",")
    ...
    data1 = np.loadtxt(f'Z:\ANSYS_simulation\inductor_practice\\result\inductance.csv', delimiter=",")
    new_data1 = np.vstack((data1, temp1))
    np.savetxt(f'Z:\ANSYS_simulation\inductor_practice\\result\inductance.csv',new_data1,delimiter=",")

    data2 = np.loadtxt(f'Z:\ANSYS_simulation\inductor_practice\\result\loss.csv', delimiter=",")
    new_data2 = np.vstack((data2, temp2))
    np.savetxt(f'Z:\ANSYS_simulation\inductor_practice\\result\loss.csv',new_data2,delimiter=",")
...
for i in range(1, 10000): 
    ...
    try :
        try:
            os.remove(f'Z:\ANSYS_simulation\inductor_practice\inductor_aedt\inductor_basic.aedt.lock')
            print("remove complete")
        except:
            time.sleep(1)
            print("remove fail")

        if os.path.isfile(f'Z:\ANSYS_simulation\inductor_practice\inductor_aedt\inductor1.aedt') :
            os.remove(f'Z:\ANSYS_simulation\inductor_practice\inductor_aedt\inductor1.aedt')
            print("inductor1 remove")
        else :
            print("not exist")
        time.sleep(1)	

        shutil.copy(f'Z:\ANSYS_simulation\inductor_practice\inductor_aedt\inductor_basic.aedt',f'Z:\ANSYS_simulation\inductor_practice\inductor_aedt\inductor1.aedt')
        print("copy")
        time.sleep(1)

        dir = "Z:\ANSYS_simulation\inductor_practice"
        os.chdir(dir)
        time.sleep(1)
        ...
        shutil.rmtree(f'Z:\ANSYS_simulation\inductor_practice\inductor_aedt\inductor1.aedtresults')
        try:
            os.remove(f'Z:\ANSYS_simulation\inductor_practice\inductor_aedt\inductor1.aedt.lock')
        except:
            time.sleep(1)
    except :
        time.sleep(1)	
